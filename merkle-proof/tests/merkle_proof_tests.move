#[test_only]
module interest_merkle_proof::merkle_proof_tests;

use interest_merkle_proof::merkle_proof::{verify, verify_with_index};
...
fun test_verify() {
    ...
    assert_eq(verify!(proof, root, leaf, |x| sha3_256(x)), true);
...
fun test_verify_with_index() {
    ...
    let (pred, index) = verify_with_index!(proof, root, leaf, |x| sha3_256(x));
    assert_eq(pred, true);
    assert_eq(index, 2);
    ...
    let leaf = x"521ec18851e17bbba961bc46c70baf03ee67ebdea11a8306de39c15a90e9d2e5";
    let (pred, index2) = verify_with_index!(proof, root, leaf, |x| sha3_256(x));
    assert_eq(index2, 66);
    assert_eq(pred, true);
...
fun test_verify_bad_proof() {
    ...
    assert_eq(verify!(proof, root, leaf, |x| sha3_256(x)), false);
}
