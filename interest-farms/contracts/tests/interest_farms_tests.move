#[test_only, allow(unused_const, unused_let_mut, unused_mut_ref, deprecated_usage)]
...
const DEFAULT_USDC_REWARDS: u64 = 3_500 * POW_10_6;
...
fun test_new_farm() {
    ...
    dapp.env!(|clock, admin_witness, ipx_metadata, scenario| {
        ...
        farm.set_end_time<IPX, SUI, Test>(clock, admin_witness, 1_000_000);
        ...
        farm.set_end_time<IPX, USDC, Test>(clock, admin_witness, 1_000_000);
...
#[
    test,
    expected_failure(
        abort_code = interest_farm_errors::EInvalidEndTimestamp,
...
fun test_new_no_rewards() {
    ...
    dapp.env!(|_clock, admin_witness, ipx_metadata, scenario| {
        let mut new_request_farm = interest_farm::request_new_farm<IPX, Test>(
            interest_farm::coin_metadata_decimals(ipx_metadata),
            admin_witness,
            scenario.ctx(),
...
fun test_stake() {
    ...
    dapp.farm_env!(|farm, clock, _admin_witness, _ipx_metadata, scenario| {
        ...
        let sui_accrued_rewards_per_share3 = farm.accrued_rewards_per_share<IPX, SUI>();
        let usdc_accrued_rewards_per_share3 = farm.accrued_rewards_per_share<IPX, USDC>();

        assert_eq(sui_accrued_rewards_per_share3, expected_sui_accrued_rewards_per_share);
        assert_eq(usdc_accrued_rewards_per_share3, expected_usdc_accrued_rewards_per_share);
...
fun test_rewards_tracking() {
    ...
    dapp.add_default_farm(POW_10_9 * 2);
...
fun add_default_farm(dapp: &mut Dapp, end_time: u64) {
    ...
    farm.set_end_time<IPX, SUI, Test>(clock, admin_witness, end_time);
    farm.set_end_time<IPX, USDC, Test>(clock, admin_witness, end_time);
